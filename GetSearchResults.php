...
/*
	HentaiVN.tv API by Nico Levianth - Getting search result
	v1.0
...
if ($_GET["query"])
{
    ...
    $html = file_get_contents("https://hentaivn.tv/tim-kiem-truyen.html?key=" . $_GET["query"] . $page);
	// Create objects
    $doc = new DomDocument();
    $info = new stdClass();
    $doc->loadHTML($html);
    ...
    $display_name = strtok(substr($nodes[0]->textContent, 37) , "▼");
    ...
    if ($info->display_name == null) echo '{"error": "Invalid query"}';
    else
    {
